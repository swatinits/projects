...
#Function to make k clusters out of the pages
def makeCluster(k):
  global final_rss
  ...
  cnt = 0
  #Initialize centroid of k clusters as tfidf of random k points
  ...
  while(1):
    ...
    for page in pages:
      ...
      for i in xrange(0, k): 
        similarity_value = get_similarity(clusters[i][0], page)
    ...
    for i in xrange(0, k):       
      centroid = get_centroid(clusters[i])
      clusters[i] = (centroid, set())
    ctr+=1   
  return clusters
...
#Fix a value of k
  ...
  purity=0
  count=0
  #Random restart ( 5 times ) and pick the cluster with maximum purity
  while(count<5):
    new_clusters = makeCluster(k)
